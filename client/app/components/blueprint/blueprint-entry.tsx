...
export function BlueprintEntry({ projectId }: { projectId: string }) {
  ...
  async function createSnapshot() {
    ...
    try {
      const created = await api.createBlueprintSnapshot(projectId, graph.project.version);
      ...
      setGraph(freshGraph); setReadiness(freshReadiness); setSnapshots(ordered); setSelectedId(created.id);
    } catch (cause) {
      if (cause instanceof ApiClientError && cause.code === "version_conflict") {
        try {
          ...
          setSelectedId((current) => current && ordered.some((item) => item.id === current) ? current : ordered[0]?.id ?? null);
          setError("Graph changed before this snapshot could be created. Reloaded current version; review it, then create again.");
      ...
      } else setError("Snapshot was not created. Your existing Blueprint history is unchanged; try again.");
