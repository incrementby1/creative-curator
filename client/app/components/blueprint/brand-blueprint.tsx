...
export function BrandBlueprint({ currentProjectVersion, onExport, projectTitle, snapshot }: Props) {
  ...
  return <div className="blueprint-reading-view">
    ...
    {stale && <p className="blueprint-stale" role="status"><History aria-hidden="true" />Current graph version {currentProjectVersion} is newer than this immutable snapshot. Create a new snapshot to publish current decisions.</p>}
    <article className="blueprint-document" data-blueprint-document="true">
      ...
      <footer className="blueprint-footer"><span>{projectTitle}</span><span>Snapshot {sequence(snapshot.sequence)} · Graph version {snapshot.project_version} · {date(snapshot.created_at)}</span></footer>
    </article>
  </div>;
}
