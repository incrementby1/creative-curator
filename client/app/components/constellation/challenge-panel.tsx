...
export function ChallengePanel({ challenge, dependencies, historyHref, resolution, onResolve }: { challenge: GraphNode; dependencies: readonly string[]; historyHref: string; resolution?: ChallengeResolution | null; onResolve: (state: Extract<ChallengeState, "resolved" | "deferred" | "overridden">, note: string) => Promise<void> }) {
  ...
  const confidence = challenge.challenge_confidence; const downstream = challenge.challenge_downstream_effect ?? "No downstream effect recorded";
  return <section aria-label={resolution ? "Resolved challenge" : "Active challenge"} className="constellation-panel challenge-panel"><header><p>{resolution ? "Challenge history" : "Active challenge"}</p><h2>{challenge.title}</h2></header><h3>Rationale</h3><p>{challenge.content}</p><h3>Dependencies</h3>{dependencies.length ? <ul>{dependencies.map((item) => <li key={item}>{item}</li>)}</ul> : <p>None linked.</p>}
    ...
    {resolution ? <div className="challenge-resolution"><strong>{label(resolution.state)}</strong><p>{resolution.resolution}</p><p>Resolved by {resolution.resolved_by ?? "unknown"} · <time dateTime={resolution.created_at}>{new Date(resolution.created_at).toLocaleString()}</time></p><a href={historyHref}>View in history</a></div> : <><label>Resolution note<textarea value={note} onChange={(e) => setNote(e.target.value)} /></label>{error && <p role="alert">{error}</p>}<div className="panel-actions"><button disabled={busy} onClick={() => void decide("resolved")} type="button">Resolve</button><button disabled={busy} onClick={() => void decide("deferred")} type="button">Defer</button><button disabled={busy} onClick={() => void decide("overridden")} type="button">Override</button></div></>}
