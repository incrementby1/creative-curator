...
export function ChallengePanel({ challenge, dependencies, historyHref, resolutions = [], onResolve }: { challenge: GraphNode; dependencies: readonly string[]; historyHref: string; resolutions?: readonly ChallengeResolution[]; onResolve: (state: Extract<ChallengeState, "acknowledged" | "resolved" | "deferred" | "overridden">, note: string) => Promise<void> }) {
  ...
  const resolution = resolutions.find((item) => ["resolved", "deferred", "overridden"].includes(item.state)); const acknowledged = resolutions.some((item) => item.state === "acknowledged"); const confidence = challenge.challenge_confidence; const downstream = challenge.challenge_downstream_effect ?? "No downstream effect recorded";
  return <section aria-label={resolution ? "Resolved challenge" : "Active challenge"} className="constellation-panel challenge-panel"><header><p>{resolution ? "Challenge history" : "Active challenge"}</p><h2>{challenge.title}</h2></header><h3>Rationale</h3><p>{challenge.content}</p><h3>Dependencies</h3>{dependencies.length ? <ul>{dependencies.map((item) => <li key={item}>{item}</li>)}</ul> : <p>None linked.</p>}
    ...
    {resolution ? <a href={historyHref}>View resolution history</a> : <>{acknowledged ? <p role="status">Acknowledged; still open.</p> : <button disabled={busy} onClick={() => { setBusy(true); void onResolve("acknowledged", "Acknowledged for review").then(() => setAnnouncement("Challenge acknowledged")).catch(() => setError("Challenge update failed.")).finally(() => setBusy(false)); }} type="button">Acknowledge</button>}<label>Resolution note<textarea value={note} onChange={(e) => setNote(e.target.value)} /></label>{error && <p role="alert">{error}</p>}<div className="panel-actions"><button disabled={busy} onClick={() => void decide("resolved")} type="button">Resolve</button><button disabled={busy} onClick={() => void decide("deferred")} type="button">Defer</button><button disabled={busy} onClick={() => void decide("overridden")} type="button">Override</button></div></>}
