...
export function ConflictPanel({ submitted, latest, submittedVersion, latestVersion, onKeepMine, onAcceptLatest, onClose }: {
  ...
  const [confirming, setConfirming] = useState(false); const confirmRef = useRef<HTMLButtonElement>(null); const comparisonRef = useRef<HTMLDivElement>(null);
  ...
  return <section aria-label="Version conflict" className="constellation-panel conflict-panel" role="alertdialog" aria-modal="false">
    ...
    {confirming && <div aria-label="Confirm keep mine" role="dialog"><p>Retry submitted values against latest version {latestVersion}?</p><button ref={confirmRef} type="button" onClick={() => void onKeepMine()}>Confirm keep mine</button><button type="button" onClick={() => setConfirming(false)}>Cancel</button></div>}
  </section>;
}
