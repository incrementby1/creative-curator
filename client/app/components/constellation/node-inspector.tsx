...
export function NodeInspector({ node, connections, revisions, loadingRevisions, availableNodes = [], onConnect, onSave }: { node: GraphNode; connections: readonly string[]; revisions: readonly NodeRevision[]; loadingRevisions: boolean; availableNodes?: readonly GraphNode[]; onConnect?: (targetId: string, edgeType: EdgeType) => Promise<void>; onSave: (input: NodeUpdateInput) => Promise<void> }) {
  ...
  return <section aria-label="Node inspector" className="constellation-panel node-inspector"><header><p>Inspector</p><h2>{node.title}</h2></header>
    <form onSubmit={save}><label>Node title<input value={title} onChange={(e) => setTitle(e.target.value)} /></label><label>Content<textarea value={content} onChange={(e) => setContent(e.target.value)} /></label>
