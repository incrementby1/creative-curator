...
const edit = (owner = "owner-a", project = "project-a", index = 1) => ({
  ...
  payload: { nodeId: "node-a", title: `Title ${index}`, content: "Exact submitted value" }, createdAt: index,
});
...
describe("PendingEditStore", () => {
  ...
  it("replays in order, clears successes, and stops at conflict", async () => {
    ...
    expect(result).toEqual({ replayed: 1, conflict: expect.objectContaining({ expectedVersion: 2 }) });
