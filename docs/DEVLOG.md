...
Measured theme contracts enforce 4.5:1 text and 3:1 control-boundary/focus contrast. Theme disclosure now manages initial focus, Escape/outside/explicit close, and trigger restoration. Browser coverage compares invariant geometry and focus/status order across Paper, Graphite, and Project at 375, 768, 1024, and 1440 pixels, plus reduced-motion and stable first themed paint.

## 2026-07-27 — Adaptive brand project entry

Authenticated root now opens owner-scoped Projects. Project home uses one bounded summary request. Persistent mode backs it with one service-role-only transactional RPC that locks the owned project page and aggregates complete nodes/resolutions in one snapshot, avoiding drift, PostgREST truncation, per-project/per-challenge N+1 reads, and edge loading. Compact rows report update time, Blueprint readiness, terminal-resolution-aware unresolved challenge count, and direct open action; failed batches expose unavailable/retry state instead of fabricated status. Empty state leads into project creation. Diagnostic draft hydration mounts a user-keyed form only after identity resolves and synchronously initializes its scoped local draft, preventing late hydration from overwriting typing. Diagnostic accepts partial intent, facts, assumptions, constraints, outcomes, and open questions, supports skip and local save/return, and never fills omitted answers. It validates a 12-node total and 500-character per-entry cap before any mutation. Creation persists project first, then sequential typed user-provenance graph nodes under optimistic project versions. Partial seeding keeps only unsent entries in user/new and user/project scoped recovery copies with direct created-project link. Protected routing, same-owner recovery, direct cross-owner graph denial, typed seeding, zero-node skip, bounded input, failure recovery, draft continuity, and mobile navigation have real FastAPI/Playwright coverage. No provider or remote Supabase operation ran.
...
## 2026-07-27 — Spatial workspace contract synchronized
