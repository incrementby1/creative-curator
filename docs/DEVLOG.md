...
Authenticated root now opens owner-scoped Projects. Compact project rows report update time, Blueprint readiness, terminal-resolution-aware unresolved challenge count, and direct open action; failed summaries expose unavailable/retry state instead of fabricated status. Empty state leads into project creation. Adaptive diagnostic accepts partial intent, facts, assumptions, constraints, outcomes, and open questions, supports skip and local save/return, and never fills omitted answers. Creation persists project first, then sequential typed user-provenance graph nodes under optimistic project versions. Partial seeding keeps only unsent entries in user/new and user/project scoped recovery copies with direct created-project link. Protected routing, same-owner recovery, direct cross-owner graph denial, typed seeding, zero-node skip, failure recovery, draft continuity, and mobile navigation have real FastAPI/Playwright coverage. No provider or Supabase operation ran.
