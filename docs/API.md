...
Media upload uses raw bytes with `Content-Type` and `X-Filename`. Streaming stops at the first chunk above 5 MiB before persistence. Spoofed/unsupported types return safe `415`; oversized input returns safe `413`. Failed attachment of a new media annotation removes the orphan upload. Downloads expose authorized bytes and detected MIME only. Project theme overrides global theme; `{"theme":null}` clears the override.
