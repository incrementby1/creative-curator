...
Supported API-key provider metadata is a static compatibility snapshot of the official Hermes Agent repository at commit `8208fc52701332f213e6c51ebc0b610be00300de`. The validated manifest records credential aliases, endpoint override names, transports, model-discovery capabilities, and provider-specific dispatch rules; it contains no credential values. Internal structured routing can dispatch injected HTTP calls through chat, Responses, Anthropic, Gemini, and Copilot adapters, with one same-provider JSON repair and ordered configured fallbacks. Each request resolves its hostname exactly once, rejects any non-global answer, and connects through a numeric pinned-IP URL while preserving the original `Host` authority and TLS hostname verification through HTTPX/httpcore's `sni_hostname` request extension. Owned clients ignore environment proxies and disable connection reuse so a pinned IP cannot share a TLS connection across provider hostnames. Responses stream into a bounded buffer that stops at the size cap (and rejects an oversized `Content-Length` before reading), and redirects remain disabled. Provider bodies, keys, and raw exceptions are excluded from typed failures. The dispatcher can be closed directly or as a context manager; application shutdown wiring remains pending with the creative-agent integration. Credential/routing HTTP settings and client UI remain unwired, and creative agents remain deterministic, so normal application use does not make live AI calls yet. The snapshot deliberately excludes LM Studio and other local no-key providers, OAuth/device-code providers, AWS SDK credential chains, and external-process providers. Updating compatibility requires reviewing the official Hermes registry, provider plugins, and docs at a new immutable commit, updating manifest and tests together, and rerunning the backend suite.
