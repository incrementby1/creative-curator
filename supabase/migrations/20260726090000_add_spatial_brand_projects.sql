...
create or replace function public.create_brand_edge(p_user_id uuid,p_project_id uuid,p_record jsonb,p_expected_project_version bigint) returns setof public.brand_edges language plpgsql security definer set search_path='' as $$ declare x public.brand_edges; begin perform public.lock_brand_project(p_user_id,p_project_id); x:=jsonb_populate_record(null::public.brand_edges,p_record); if x.user_id<>p_user_id or x.project_id<>p_project_id or x.version<>1 or not exists(select 1 from public.brand_nodes where user_id=p_user_id and project_id=p_project_id and id=x.source_node_id and state<>'trash') or not exists(select 1 from public.brand_nodes where user_id=p_user_id and project_id=p_project_id and id=x.target_node_id and state<>'trash') then raise exception 'invalid_edge' using errcode='23514'; end if; update public.brand_projects set version=version+1 where user_id=p_user_id and id=p_project_id and version=p_expected_project_version; if not found then raise exception 'version_conflict' using errcode='40001'; end if; return query insert into public.brand_edges select x.* returning *; end $$;
create or replace function public.update_brand_edge(p_user_id uuid,p_project_id uuid,p_record jsonb,p_expected_edge_version bigint,p_expected_project_version bigint) returns setof public.brand_edges language plpgsql security definer set search_path='' as $$ declare x public.brand_edges; changed public.brand_edges; begin perform public.lock_brand_project(p_user_id,p_project_id); x:=jsonb_populate_record(null::public.brand_edges,p_record); if x.user_id<>p_user_id or x.project_id<>p_project_id or x.version<>p_expected_edge_version+1 or not exists(select 1 from public.brand_nodes where user_id=p_user_id and project_id=p_project_id and id=x.source_node_id and state<>'trash') or not exists(select 1 from public.brand_nodes where user_id=p_user_id and project_id=p_project_id and id=x.target_node_id and state<>'trash') then raise exception 'invalid_edge' using errcode='23514'; end if; update public.brand_edges e set source_node_id=x.source_node_id,target_node_id=x.target_node_id,edge_type=x.edge_type,label=x.label,version=x.version,updated_at=x.updated_at where e.user_id=p_user_id and e.project_id=p_project_id and e.id=x.id and e.version=p_expected_edge_version returning e.* into changed; if changed.id is null then raise exception 'version_conflict' using errcode='40001'; end if; update public.brand_projects set version=version+1 where user_id=p_user_id and id=p_project_id and version=p_expected_project_version; if not found then raise exception 'version_conflict' using errcode='40001'; end if; return next changed; end $$;
