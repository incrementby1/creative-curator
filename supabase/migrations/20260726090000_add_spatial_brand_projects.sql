...
create or replace function public.save_brand_layout(p_user_id uuid,p_project_id uuid,p_positions jsonb,p_dimensions jsonb,p_expected_version bigint) returns bigint language plpgsql security definer set search_path='' as $$ declare v bigint; begin perform public.lock_brand_project(p_user_id,p_project_id); if jsonb_typeof(p_positions)<>'object' or exists(select 1 from jsonb_each(p_positions) where key='' or jsonb_typeof(value)<>'array' or jsonb_array_length(value)<>2 or jsonb_typeof(value->0)<>'number' or jsonb_typeof(value->1)<>'number') or jsonb_typeof(p_dimensions)<>'object' or exists(select 1 from jsonb_each(p_dimensions) where key='' or jsonb_typeof(value)<>'array' or jsonb_array_length(value)<>2 or jsonb_typeof(value->0)<>'number' or jsonb_typeof(value->1)<>'number' or (value->>0)::numeric<80 or (value->>0)::numeric>1200 or (value->>1)::numeric<64 or (value->>1)::numeric>900) then raise exception 'invalid_layout' using errcode='23514'; end if; select version into v from public.brand_layouts where user_id=p_user_id and project_id=p_project_id for update; if v is null and p_expected_version<>0 then raise exception 'version_conflict' using errcode='40001'; end if; if v is null then insert into public.brand_layouts(user_id,project_id,positions,dimensions,version) values(p_user_id,p_project_id,p_positions,p_dimensions,1); elsif v<>p_expected_version then raise exception 'version_conflict' using errcode='40001'; else update public.brand_layouts set positions=p_positions,dimensions=p_dimensions,version=v+1,updated_at=now() where user_id=p_user_id and project_id=p_project_id; end if; return p_expected_version+1; end $$;
